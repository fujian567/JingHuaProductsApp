...
@IonicPage()
@Component({
  selector: 'page-modifymobile',
  templateUrl: 'modifymobile.html',
})
export class ModifymobilePage {
  ...
  postNewPwd() {
    this.appService.httpPost_token(AppGlobal.API.postClientRestPwd, this.c_token, this.FindPwdViewModel, rs => {
      if (rs.isSuccess) {
        this.appConfigCtrl.popAlertView('修改成功！')
      } else {
        console.log(rs)
  ...
  resetpwd() {
    if (this.FindPwdViewModel.Account == "") {
      ...
      this.appConfigCtrl.popAlertView('请填写新手机号');
      return
    } else if (!AppStaticConfig.verifyMobile(this.FindPwdViewModel.NewPassword)) {
      this.appConfigCtrl.popAlertView('您填写的新手机号不符合格式，请重新填写');
      return
    } else {
      this.postNewPwd();
    }
